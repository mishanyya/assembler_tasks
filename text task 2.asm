...
lm equ $-message        ;lm - длина сообщения
                        ;0xa - перенос строки
                        ;0 - не обязателен, но может понадобиться при работе с C
...
;начать вывод с последней ячейки памяти, т.е. наоборот
...
sub ebx,1              ;уменьшаем, так как если существует 10 ячеек с №№ 1 по 10,
                       ;то к 1 номеру + 10  получается несуществующая ячейка №11
mov eax,out            ;поместить адрес можно только в 4-байтный регистр, например eax!

again:                       ;метка начала цикла

                      ;в eax и ebx уже находятся номера адресов enter и out,
                      ;поэтому можно работать сразу с ними не трогая исходные enter и out
mov dl,byte[ebx]      ;поместить содержимое по адресу ebx в 1-байтовый регистр dl
mov byte[eax],dl      ;присвоить адрес следующего символа следующей ячейке памяти out
                      ;из регистра dl

dec ebx               ;уменьшить регистр с адресом на 1
inc eax               ;увеличить регистр с адресом на 1

loop again      ; количество повторений содержится в регистре ecx
...
mov eax, 1          ;в регистр в EAX помещается 1 - номер системного вызова "exit"
mov ebx, 0          ;в регистр в EBX помещается 0 - параметр вызова "exit"
int 0x80            ;системный вызов. После системного вызова "exit"

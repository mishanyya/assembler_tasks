...
error1  db "Нет аргументов в командной строке!",0xa,0 ;+
lerror1 equ $-error1

error2  db "Файл не открыт!",0xa,0 ;+
lerror2 equ $-error2

error4  db "Большой размер файла, он превышает 255 байт!",0xa,0 ;+
lerror4 equ $-error4

error9  db "Файл не закрыт!",0xa,0 ;+
lerror9 equ $-error9
...
;вывод ошибки 1, если введен только 1 параметр, т.е. только исполняемый файл без других аргументов
cmp bl,'1'
je er1
jne next1
er1:
 mov ecx,error1   ;ввод полученного адреса памяти от mmap
 mov edx,lerror1      ;ввод количества символов
 mov eax,4
 mov ebx,1
 int 80h
 jmp exit
...
mov ecx,100       ;счетчик
mov esi,filetoopen  ;в esi адрес первого символа
...
mov edx,lfiletoopen      ;ввод количества символов
mov eax,4
mov ebx,1
int 80h
...
jne next2
er2:
 mov ecx,error2   ;ввод полученного адреса памяти от mmap
 mov edx,lerror2      ;ввод количества символов
 mov eax,4
 mov ebx,1
 int 80h
 jmp exit
...
;выводит все символы, но
;адрес в [edi] не может быть отображен
...
;сравнить dl с 256
 ...
 jle next4
 er4:
  mov ecx,error4   ;ввод полученного адреса памяти от mmap
  mov edx,lerror4      ;ввод количества символов
  mov eax,4
  mov ebx,1
  int 80h
  jmp exit
...
jge next9
er9:
 mov ecx,error9   ;ввод полученного адреса памяти от mmap
 mov edx,lerror9      ;ввод количества символов
 mov eax,4
 mov ebx,1
 int 80h
 jmp exit

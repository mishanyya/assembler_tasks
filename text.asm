...
mov ch,42 ;*        ;номер в регистре обычно обозначает номер символа в ASCII
cmp cl,ch
je yesmul            ;а затем в обратную двоичную форму

;деление работает
mov cl,byte[sign]   ;перевести числовое значение из byte[sign] в cl
mov ch,47  ;/        ;номер в регистре обычно обозначает номер символа в ASCII
cmp cl,ch
je yesdiv          ;а затем в обратную двоичную форму

;сложение работает
mov cl,byte[sign]   ;перевести числовое значение из byte[sign] в cl
mov ch,43   ;+        ;номер в регистре обычно обозначает номер символа в ASCII
cmp cl,ch
je yesadd            ;а затем в обратную двоичную форму

;вычитание пока не работает
mov cl,byte[sign]   ;перевести числовое значение из byte[sign] в cl
mov ch,45  ;-        ;номер в регистре обычно обозначает номер символа в ASCII
cmp cl,ch
je yessub            ;а затем в обратную двоичную форму

jmp tonosign   ;если не найдено
...
;операция сложения
yesadd:
add esi,eax    ;сложить регистры, а результат в esi
mov ax,si      ;меньшие 2 байта поместить в ax
shr esi,16     ;сдвинуть значение в esi на 16 битов
mov dx,si      ;старшие 2 бита от суммы esi поместить в dx

;вывод результата деления из регистра ax в память output
mov edi,output   ;получить адрес output
mov cx,6  ;счетчик, так как если максим сумма для чисел 65535 поместится в 6 знаков
mov si,10  ;для вывода в 10-м формате
...
;операция вычитания
yessub:
sub ax,si   ;высчитать из ax значение si, а результат в ax
jnc nominus    ;если нет переноса
jc minus       ;если есть перенос


;;;;;;;; ;;;;;;;;
minus:
;перевод из положительного числа в отрицательное
mov bx,ax
not bx
inc bx
mov ax,bx
;и надо добавить в ответ знак 'минус'
;так как вывод идет с последнего символа, знак надо



;Вычитание выполняется с помощью команды SUB. Результат также
;помещается на место первого операнда и опять же выставляются флаги.
...
;На самом деле вычитание в процессоре реализовано с помощью сложения.
 ;Процессор меняет знак второго операнда на противоположный, а
 ;затем складывает два числа. Если вам необходимо в программе
 ;поменять знак числа на противоположный, можно использовать
 ;команду NEG. У этой команды всего один операнд.
;при вычитании и получении минусового значения
;флаг знака не выставляется
;а выставляется флаг переполнения
;потому что процессор складывает эти числа, а не высчитывает
;и если в ответе получается отрицательное число,
;то изменится флаг переноса CF, а не флаг знака SF!
;но если число сравнительно небольшое, то сработает флаг знака SF,
;поэтому надо ориентироваться на один из этих флагов, т.е. SF или CF
...
;вывод знака '-'
mov ecx,minus1       ;ввод адреса
mov edx,lminus1     ;ввод количества символов
call PRINT

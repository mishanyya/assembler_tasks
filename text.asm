...
filetoopen db "forsearch.txt",0
lfiletoopen equ $-filetoopen

n db 0xa                   ;переменная для переноса строки

;filetoopen db 'forsearch.txt',0        ;адрес файла
...
;После выполнения команды, регистры SI и DI увеличиваются
;на 1, если флаг DF = 0, или уменьшаются на 1, если DF = 1.
...
;2 - Открывает файл и для чтения и для записи.
;Аргумент flags может также включать любую комбинацию следующих флагов:
;2000 запишут данные в конец файла, расширяя его, независимо от текущей файловой позиции.
;100 будет создан файл, если он еще не существует.
;200 Если и 100 и 200 установлены, то open выдает ошибку, если заданный файл уже существует.
;400 Если filename — имя терминала, не делайте его терминалом управления для процесса.
;4000 Устанавливает режим неблокирования.
;1000 Если файл существует и открыт для записи, он усекается до нулевой длины

;Нуль-терминированная строка или C-строка или ASCIIZ-строка — в
;которой концом строки считается первый встретившийся специальный
;нуль-символ (NUL из кода ASCII, со значением 0).
